...
What happens when you update environment variables?

Many of the environment variables make their way into Terraform files, so when you
change your `environment` or `environment.local` file, it is not enough to run

```
source environment
```

You must also remake the `variables.tf` files in the `infra/` folder. These `variables.tf`
files are automatically generated by the `infra/build_deploy_config.py` script. To remake
these files:
...
#### Destroying or Renaming Resources

What happens when resources already exist from a prior deployment?

Here, we have two options:

1. Delete and re-create all resources

1. Import all existing resources

**Deleting Resources:**

To delete resources in `infra/`, you can use the Makefile to destroy resources.
...
`make -C infra destroy-all` command will not work.
